...
getgenv().AristoisInjected = true

function Aristois.UnInject()
    game:GetService("CoreGui")['Aristois']:Destroy()
...
if not _G.Aristois.GuiDetectionBypass then
    ...
    _G.Aristois.GuiDetectionBypass = true
end
...
function Aristois.newNotification(notifType: string, heading: string, body: string, autoRemove: boolean, autoRemoveTime: number, callback): table
    ...
    local newNotif = setmetatable({}, Aristois)
...
function Aristois:changeHeading(newHeading: string): nil
    coroutine.wrap(function()
...
function Aristois:changeBody(newBody: string): nil
    coroutine.wrap(function()
        if not self.Instance or not self.Instance:FindFirstChild("templateFrame") then return end
...
function Aristois:deleteTimeout(waitTime: number): nil
    coroutine.wrap(function()
        if not self.Instance or not self.Instance:FindFirstChild("templateFrame") then return end
